...
def parse_args():
    ...
    config_or_template = config_options.add_mutually_exclusive_group(required=False)
    ...
    ############################################################
    # Fronting options
    fronting_test_grp = parser.add_argument_group(_title("Fronting speed test options"))
    ft_or_nofronting = fronting_test_grp.add_mutually_exclusive_group(required=False)
    ...
    ############################################################
    # upload options
    upload_speed_grp = parser.add_argument_group(_title("Upload speed test options"))
    ...
    if parsed_args.sample_size is not None:
        if 0 < parsed_args.sample_size < 1:
            ...
            raise ValueError(
                color_text("Sample size must be a positive number.", rgb=(255, 0, 0))
            )
