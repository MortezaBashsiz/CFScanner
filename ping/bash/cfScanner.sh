		...
		do
			...
			then
				...
				then
					...
					then
						finalResult="$timeMil,$provider,$ip"
            curlResult=$(curl -s -XPOST http://bot.sudoer.net:8452/cfiran/a50ced64-6eba-4ab8-a30d-3a1f166a5135/"$provider"/"$finalResult")
						if [[ "$curlResult" == "Done" ]]
						then
...
function fncMainCFFindIP {
	...
	cloudFlarePartList=$(curl -s -XGET http://bot.sudoer.net/part-all)
	for part in ${cloudFlarePartList}
	do
	  cloudFlareIpList=$(curl -s -XGET http://bot.sudoer.net/"$part")
	done
	for part in ${cloudFlarePartList}
	do
	  cfIPList=$(curl -s -XGET http://bot.sudoer.net/"$part")
		killall v2ray > /dev/null 2>&1
		tput cuu1; tput ed # rewrites Parallel's bar
		if [[ $parallelVersion -gt "20220515" ]];
		then
		  parallel --ll --bar -j "$threads" fncCheckIPList ::: "$cfIPList" ::: "$progressBar" ::: "$resultFile" ::: "$scriptDir" ::: "$configId" ::: "$configHost" ::: "$configPort" ::: "$configPath" ::: "$configServerName" ::: "$frontDomain" ::: "$scanDomain" ::: "$downloadFile" ::: "$osVersion" ::: "$v2rayCommand" ::: "$provider"
		else
		  echo -e "${RED}$progressBar${NC}"
		  parallel -j "$threads" fncCheckIPList ::: "$cfIPList" ::: "$progressBar" ::: "$resultFile" ::: "$scriptDir" ::: "$configId" ::: "$configHost" ::: "$configPort" ::: "$configPath" ::: "$configServerName" ::: "$frontDomain" ::: "$scanDomain" ::: "$downloadFile" ::: "$osVersion" ::: "$v2rayCommand" ::: "$provider"
		fi
		killall v2ray > /dev/null 2>&1
	done
}
...
providerList=(IRC MCI RTL SHT ZTL AST MBT MKB PRS HWB TEST)
if [[ ! "${providerList[*]}" =~ $provider ]]
then
  echo "choose one of ${providerList[*]}"
  exit 1
fi

if [[ "$mode" == "IP" ]]
then
	fncMainCFFindIP	"$threads" "$progressBar" "$resultFile" "$scriptDir" "$configId" "$configHost" "$configPort" "$configPath" "$configServerName" "$frontDomain" "$scanDomain" "$speed" "$osVersion" "$provider"
else
