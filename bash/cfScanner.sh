...
fncSubnetToIP() {
    ...
    # Handle the /32 case separately
    if [[ ${mask} == 32 ]]; then
        ipList+=("${network[0]}")
    else
        # Generate IP list for subnets smaller than /32 using existing loop logic
        local bytes=(0 0 0 0)
    ...
    # Process the generated IP list based on randomNumber
    if [[ "$randomNumber" != "NULL" ]]; then
        # Choose and print a random sample of IPs
        ...
        for ip in ${ipList}; do
            ...
            if $timeoutCommand 1 bash -c "</dev/tcp/$ip/443" >/dev/null 2>&1; then
                ...
                if [[ "$domainFronting" == "0000000000" ]]; then
                    ...
                    if [[ $downSuccessedCount -ge $downThreshold ]] && [[ "$downloadOrUpload" != "UP" ]]; then
                        downOK="YES"
                        downRealTime=$((downTotalTime / downSuccessedCount))
                    else
                        downRealTime=0
                    fi
                    if [[ $upSuccessedCount -ge $upThreshold ]] && [[ "$downloadOrUpload" != "DOWN" ]]; then
                        upOK="YES"
...
function fncCheckDpnd {
    ...
    if [[ "$(uname)" == "Linux" ]]; then
        command -v jq >/dev/null 2>&1 || {
            echo >&2 "I require 'jq' but it's not installed. Please install it and try again."
            kill -s 1 "$TOP_PID"
        }
        command -v parallel >/dev/null 2>&1 || {
            echo >&2 "I require 'parallel' but it's not installed. Please install it and try again."
            kill -s 1 "$TOP_PID"
        }
        command -v bc >/dev/null 2>&1 || {
            echo >&2 "I require 'bc' but it's not installed. Please install it and try again."
            kill -s 1 "$TOP_PID"
        }
        command -v timeout >/dev/null 2>&1 || {
            echo >&2 "I require 'timeout' but it's not installed. Please install it and try again."
            kill -s 1 "$TOP_PID"
        }
        osVersion="Linux"
        ...
        command -v gtimeout >/dev/null 2>&1 || {
            echo >&2 "I require 'gtimeout' but it's not installed. Please install it and try again."
            kill -s 1 "$TOP_PID"
        }
        osVersion="Mac"
    fi
...
function fncMainCFFindSubnet {
    ...
    for subNet in ${cfSubnetList}; do
        ...
        for breakedSubnet in ${breakedSubnets}; do
            ...
            tput cuu1
            tput ed # rewrites Parallel's bar
            if [[ $parallelVersion -gt 20220515 ]]; then
                parallel --ll --bar -j "$threads" fncCheckIPList ::: "$ipList" ::: "$progressBar" ::: "$resultFile" ::: "$scriptDir" ::: "$configId" ::: "$configHost" ::: "$configPort" ::: "$configPath" ::: "$fileSize" ::: "$osVersion" ::: "$clientCommand" ::: "$tryCount" ::: "$downThreshold" ::: "$upThreshold" ::: "$downloadOrUpload" ::: "$vpnOrNot" ::: "$quickOrNot"
            else
                echo -e "${RED}$progressBar${NC}"
                parallel -j "$threads" fncCheckIPList ::: "$ipList" ::: "$progressBar" ::: "$resultFile" ::: "$scriptDir" ::: "$configId" ::: "$configHost" ::: "$configPort" ::: "$configPath" ::: "$fileSize" ::: "$osVersion" ::: "$clientCommand" ::: "$tryCount" ::: "$downThreshold" ::: "$upThreshold" ::: "$downloadOrUpload" ::: "$vpnOrNot" ::: "$quickOrNot"
            fi
            killall v2ray >/dev/null 2>&1
...
function fncMainCFFindIP {
    ...
    tput ed # rewrites Parallel's bar
    if [[ $parallelVersion -gt 20220515 ]]; then
        parallel --ll --bar -j "$threads" fncCheckIPList ::: "$cfIPList" ::: "$progressBar" ::: "$resultFile" ::: "$scriptDir" ::: "$configId" ::: "$configHost" ::: "$configPort" ::: "$configPath" ::: "$fileSize" ::: "$osVersion" ::: "$clientCommand" ::: "$tryCount" ::: "$downThreshold" ::: "$upThreshold" ::: "$downloadOrUpload" ::: "$vpnOrNot" ::: "$quickOrNot"
    else
        echo -e "${RED}$progressBar${NC}"
        parallel -j "$threads" fncCheckIPList ::: "$cfIPList" ::: "$progressBar" ::: "$resultFile" ::: "$scriptDir" ::: "$configId" ::: "$configHost" ::: "$configPort" ::: "$configPath" ::: "$fileSize" ::: "$osVersion" ::: "$clientCommand" ::: "$tryCount" ::: "$downThreshold" ::: "$upThreshold" ::: "$downloadOrUpload" ::: "$vpnOrNot" ::: "$quickOrNot"
    fi
    killall v2ray >/dev/null 2>&1
    sort -n -k1 -t, "$resultFile" -o "$resultFile"
}
...
function fncUsage {
    if [[ "$osVersion" == "Mac" ]]; then
      ...
      [ -h ] help\n"
        ...
        echo -e "Usage: cfScanner [ -x|--core V2RAY/XRAY ]
      [ -v|--vpn-mode YES/NO ]
      [ -m|--mode  SUBNET/IP ]
