...
data class V2rayConfig(
    ...
    var inbounds: ArrayList<InboundBean>,
    ...
    fun getByCustomInbound(port: Int, address: String = "127.0.0.1"): V2rayConfig {
        return this.apply {
            inbounds.filter { it.tag == "socks" }.forEach {
                it.listen = address
                it.port = port
            }
        }
    ...
    fun getByCustomVnextOutbound(port: Int, address: String): V2rayConfig {
        return this.apply {
    ...
    fun isVmessVlessConnection():Boolean{
        outbounds.filter { it.tag == "proxy" }.forEach {
            if(it.protocol.contains("vless",false) || it.protocol.contains("vmess",false) ){
                return true
            }
        }
        return false
